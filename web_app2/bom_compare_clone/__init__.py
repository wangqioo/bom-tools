...
bom_compare_clone_bp = Blueprint('bom_compare_clone', __name__)
...
def _item_value(item, names):
    if not item:
        return ''
    headers = item.get('headers') or HQ_STANDARD_HEADERS
    raw = item.get('raw') or []
    ...
    for name in names:
        ...
        if idx is not None and idx < len(raw):
            value = _cell_str(raw[idx])
            if value:
                return value
    return ''


def _item_full_attrs(item):
    if not item:
        return ''
    headers = item.get('headers') or HQ_STANDARD_HEADERS
    raw = item.get('raw') or []
...
def _diff_category(field):
    norm = _normalize_header(field)
    if norm in {_normalize_header(v) for v in ('位号', 'refdes', 'reference')}:
        return '位号变更'
    if norm in {_normalize_header(v) for v in ('用量', '单耗', '数量', 'qty')}:
        return '用量变更'
    if norm in {_normalize_header(v) for v in ('制程', '工艺', 'process')}:
        return '制程变更'
    if norm in {_normalize_header(v) for v in ('是否量产标识', '量产标识', '量产', 'massproduction')}:
        return '量产标识变更'
    if norm in {_normalize_header(v) for v in ('型号', '规格型号', 'model')}:
        return '型号变更'
    if norm in {_normalize_header(v) for v in ('生产厂家', '制造商', '厂商', 'manufacturer')}:
        return '厂商变更'
    if norm in {_normalize_header(v) for v in ('替代关系', '主辅料', '替代')}:
        return '替代关系变更'
    return '字段变更'


def _change_values(old, new, field, removed_refdes=None, added_refdes=None):
    if field == '位号':
        return '、'.join(removed_refdes or []), '、'.join(added_refdes or [])
    return (
    ...
    for key in all_keys:
        ...
        if old and not new:
            ...
            for field in changed_fields:
                category = _diff_category(field)
                if category not in diff_categories:
                    diff_categories.append(category)
            if diff_categories:
                diff_type = '变更'
            else:
                diff_type = '未变更'
...
def _write_diff_report(out_path, old_rows, new_rows, compare_cols, stats, duplicate_notes, old_meta=None, new_meta=None):
    ...
    for ri, (name, value) in enumerate(summary_rows, 3):
        ...
        ws.cell(row=ri, column=1).border = bdr
        ws.cell(row=ri, column=2).border = bdr
    legend = [('绿色', '新增'), ('红色', '删除'), ('黄色', '多字段差异'), ('灰色', '未变更')]
    ...
    detail_headers = [
        '差异类型合集', '料号', '基准版本行号', '对比版本行号',
        '变更字段合集', '基准变更值', '对比变更值',
        '基准用量', '对比用量', '基准位号', '对比位号',
        '基准制程', '对比制程', '基准是否量产标识', '对比是否量产标识',
        '基准型号', '对比型号', '基准物料描述', '对比物料描述',
        '基准替代关系', '对比替代关系', '基准生产厂家', '对比生产厂家',
    ]
    detail = wb.create_sheet('差异明细')
    for ci, header in enumerate(detail_headers, 1):
        c = detail.cell(row=1, column=ci, value=header)
        c.font = Font(bold=True)
        c.alignment = center
        c.border = bdr
        c.fill = header_fill

    def combined_changed_values(item):
        old = item['old']
        new = item['new']
        old_parts = []
        new_parts = []
        for field in item.get('changed_fields') or []:
            old_value, new_value = _change_values(old, new, field, item.get('removed_refdes'), item.get('added_refdes'))
            old_parts.append(f'{field}: {old_value}')
            new_parts.append(f'{field}: {new_value}')
        return '\n'.join(old_parts), '\n'.join(new_parts)

    detail_items = [item for item in items if item.get('type') != '未变更']
    for ri, item in enumerate(detail_items, 2):
        old = item['old']
        new = item['new']
        old_changed, new_changed = combined_changed_values(item)
        categories = item.get('diff_categories') or [item['type']]
        row_fill = fills['新增' if '新增' in categories else '删除' if '删除' in categories else '未变更' if '未变更' in categories else '变更']
        row_values = [
            '；'.join(categories),
            item['key'],
            old['row'] if old else '',
            ...
            old_changed,
            new_changed,
            _item_value(old, ['用量', '单耗', '数量', 'Qty']),
            _item_value(new, ['用量', '单耗', '数量', 'Qty']),
            _item_value(old, ['位号', 'Refdes', 'Reference']),
            _item_value(new, ['位号', 'Refdes', 'Reference']),
            _item_value(old, ['制程', '工艺', 'Process']),
            _item_value(new, ['制程', '工艺', 'Process']),
            _item_value(old, ['是否量产标识', '量产标识', '量产', 'Mass Production']),
            _item_value(new, ['是否量产标识', '量产标识', '量产', 'Mass Production']),
            _item_value(old, ['型号', '规格型号', 'Model']),
            _item_value(new, ['型号', '规格型号', 'Model']),
            _item_value(old, ['物料描述', '描述', 'Description']),
            _item_value(new, ['物料描述', '描述', 'Description']),
            _item_value(old, ['替代关系', '主辅料', '替代']),
            _item_value(new, ['替代关系', '主辅料', '替代']),
            _item_value(old, ['生产厂家', '制造商', '厂商', 'Manufacturer']),
            _item_value(new, ['生产厂家', '制造商', '厂商', 'Manufacturer']),
        ]
        for ci, value in enumerate(row_values, 1):
            c = detail.cell(row=ri, column=ci, value=value)
    ...
    for sheet in wb.worksheets:
        for col in range(1, sheet.max_column + 1):
            width = 16
            if sheet.title == '差异明细' and col in (1, 5, 6, 7):
                width = 34
            elif col in (2, 10, 11, 16, 17, 18, 19, 22, 23):
                width = 24
...
@bom_compare_clone_bp.route('/api/bom_compare_clone/customer_hq_preview', methods=['POST'])
...
@bom_compare_clone_bp.route('/api/bom_compare_clone/generic_sheets', methods=['POST'])
...
@bom_compare_clone_bp.route('/api/bom_compare_clone/generic', methods=['POST'])
...
@bom_compare_clone_bp.route('/api/bom_compare_clone/local_sheets', methods=['POST'])
...
@bom_compare_clone_bp.route('/api/bom_compare_clone/machine_local_sheets', methods=['POST'])
...
@bom_compare_clone_bp.route('/api/bom_compare_clone/machine_hq_version', methods=['POST'])
def api_machine_hq_version_compare():
    ...
    try:
        ...
        return jsonify({'success': False, 'error': str(e)})
@bom_compare_clone_bp.route('/api/bom_compare_clone/hq_version', methods=['POST'])
