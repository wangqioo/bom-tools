...
SENSITIVE_KEY_RE = re.compile(r'(secret|token|apikey|api_key|authorization|ciphertext)', re.I)
EMBEDDED_SECRET_PATTERNS = [
    (
        re.compile(
            r'(?i)'
            r'(apiKey|api_key|appSecret|app_secret|accessToken|access_token|ciphertext)'
            r'(\s*[:=]\s*["\']?)([^"\',&\s}]+)'
        ),
        r'\1\2<redacted>',
    ),
...
def get_aster_log_path(log_file: str = '') -> str:
    ...
    return str(Path(__file__).resolve().parent / 'logs' / 'aster_debug.log')
...
class AsterConfig:
    ...
    def from_env(cls, environ: Optional[dict] = None) -> 'AsterConfig':
        ...
        return cls(
            ...
            base_url=str(env.get('ASTER_BASE_URL') or '').strip(),
            ...
            timeout_seconds=_env_float(env.get('PSTX_ASTER_TIMEOUT_SECONDS'), 45.0, minimum=1.0),
...
def _request_json(url: str,
    ...
    try:
        ...
        diagnostics = _diagnostics(context, response_body=_safe_text_preview(text, 1200))
...
class AsterFlowClient:
    ...
    def chat_flow(self, *, query: str, inputs: Optional[dict] = None) -> dict:
        ...
        context = self._context('chat_flow')
...
class AsterAuthClient:
    ...
    def validate_access_token(self, token: str) -> dict:
        origin = self._origin()
        if not origin:
            raise AsterConfigError('Room token 校验需要 ASTER_ORIGIN，或可从 ASTER_BASE_URL 推导出域名')
...
def _build_review_scope(report: dict) -> List[dict]:
    ...
    rows = [
        _review_scope_item(
            'BOM 与装配状态',
            'bom',
            ['bom_normal_merged', 'bom_depop_merged', 'bom_option_components'],
            report,
            f"贴装种类 {metrics.get('贴装种类', 0)}，DEPOP 总数 {metrics.get('DEPOP 总数', 0)}，include_depop={report.get('include_depop')}",
        ),
        _review_scope_item(
            '网络分类与页码映射',
            'network',
...
def build_aster_live_summary(report: dict, bundle: dict, *, environ: Optional[dict] = None) -> dict:
    ...
    request_payload_chars = len(json.dumps(brief, ensure_ascii=False))
    ...
    return normalize_aster_answer(
        ...
        bundle=bundle,
        config=live_config,
        metadata={'conversation_id': data.get('roomId') or room.get('roomId') or ''},
        request_payload_chars=request_payload_chars,
    )
