...
def build_aster_status(*, environ: Optional[dict] = None) -> dict:
    ...
    required_names = set()
    if config.mode == 'live':
        required_names.update({'ASTER_BASE_URL', 'ASTER_EMP_NO'})
    ...
    items = [
        ...
        _status_item(
            ...
            'ASTER_BASE_URL',
            'Aster 服务地址',
            required='ASTER_BASE_URL' in required_names,
            source='runtime' if 'ASTER_BASE_URL' in runtime_names else 'environment',
            safe_value=_safe_base_url(config.base_url),
        ),
        _status_item(
            env,
            'ASTER_EMP_NO',
...
def aster_error_payload(exc: Exception) -> Tuple[dict, int]:
    ...
    if '401' in error_text or 'unauthorized' in error_text.lower() or 'access token is invalid' in error_text.lower():
        hints.extend([
            ...
            '检查 ASTER_BASE_URL 是否指向 Aster wrapper 根地址，避免把 Dify 原生地址或错误路径填入。',
        ])
    payload = {
        'ok': False,
        'mode': 'live',
