...
def build_aster_mock_summary(report: dict, bundle: dict) -> dict:
    ...
    section_focus = [
        {
            ...
            'reason': f"缺料号 {_table_count(drc_section, 'missing_hq_code')}，缺 VALUE {_table_count(drc_section, 'missing_value')}，BOM_OPTION {_table_count(drc_section, 'bom_option_components')}",
        },
        {
            'section': '电阻检查',
            'target': 'resistor',
    ...
    review_checklist = [
        _checklist_item(
            'BOM 与装配状态',
            'needs_review' if _table_count(drc_section, 'bom_option_components') else 'covered_no_findings',
            f"BOM_OPTION {_table_count(drc_section, 'bom_option_components')}，DEPOP 总数 {_metric_value(report, 'DEPOP 总数', 0)}，include_depop={report.get('include_depop')}",
            'bom',
            'medium',
        ),
        _checklist_item(
