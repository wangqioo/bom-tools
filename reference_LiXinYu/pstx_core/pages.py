...
def parse_page_map_line(raw_line: str) -> Optional[Dict[str, str]]:
    """Parse one ``page.map`` line into logical/real page labels and title."""
    return _parse_page_map_line(raw_line)
...
def extract_page_number_from_text(text: str) -> str:
    """Extract a Cadence ``PAGE_NUMBER`` value from CSV/CSA-like text."""
...
def read_page_number_from_csv(csv_path: Path) -> str:
    """Read a Cadence page CSV file and return its normalized logical page."""
    return _read_page_number_from_csv(Path(csv_path))
...
def _page_number_int(page_label: object) -> int:
    match = _PAGE_TOKEN_RE.search(str(page_label or ""))
    return int(match.group(1)) if match else 0


def summarize_module_order_page_extent(project_root: str) -> Dict[str, object]:
    """Return the overall user-visible schematic page extent from module_order."""

    index = build_module_order_index(project_root)
    entries: List[Dict[str, object]] = []
    for bucket in (index.get("by_key") or {}).values():
        if isinstance(bucket, list):
            entries.extend([entry for entry in bucket if isinstance(entry, dict)])

    page_entries: List[Dict[str, object]] = []
    for entry in entries:
        start_page = _page_number_int(entry.get("start_real_page"))
        try:
        ...
        page_entries.append({
            ...
            "start_real_page": _coerce_page_number(str(start_page)),
            "page_count": page_count,
            "end_page": end_page,
            "end_real_page": _coerce_page_number(str(end_page)),
    ...
    if not page_entries:
        return {
            ...
            "total_pages": 0,
            "last_page": "",
            "entry_count": int(index.get("count", 0) or 0),
            "files": list(index.get("files") or []),
            "warnings": list(index.get("warnings") or []),
            "reason": "未从 module_order(.dat) 读到可计算的页范围。",
        }

    last_entry = max(page_entries, key=lambda item: (item["end_page"], item["start_page"], str(item.get("path_key", ""))))
    return {
        "available": True,
        "total_pages": int(last_entry["end_page"]),
        "last_page": last_entry["end_real_page"],
        "entry_count": int(index.get("count", 0) or 0),
        "file_count": len(index.get("files") or []),
        "files": list(index.get("files") or []),
        "warnings": list(index.get("warnings") or []),
        "last_entry": last_entry,
        "top_entries": sorted(page_entries, key=lambda item: (-item["end_page"], -item["start_page"], str(item.get("path_key", ""))))[:12],
    }
...
def resolve_component_page_info(
    ...
    *,
    ...
    if not submodule_mapped_page and not submodule_real_page and not lookup_candidates and final_real_page:
        submodule_mapped_page = final_real_page
        module_state = "not_submodule"
        submodule_mapping_note = "非子模块元件，页码沿用主模块解析结果"
...
def build_page_mapping_rows(
    ...
    for logical_page in sorted(logical_pages, key=_natural_sort_key):
        ...
        if page_map_state == "ambiguous" or page_csv_state == "ambiguous":
            ...
            ok = "否"
            status = "页码对应多个主模块页"
            note = f"{final_real} 同时被多个主模块页复用：{', '.join(reverse_logicals)}"
        ...
        rows.append(
            {
                "主模块页": logical_page,
                "页码": final_real,
                "page.map页码": page_map_real,
                "page*.csv页码": page_csv_real,
                "是否一一对应": ok,
                "状态": status,
                "说明": note,
            }
