...
$packagedUsersDb = Join-Path $extractDir "web_app2\auth_data\users.sqlite3"
if (Test-Path $packagedUsersDb -PathType Leaf) {
  Copy-SqliteDatabase -Source $packagedUsersDb -Destination (Join-Path $InstallDir "web_app2\auth_data\users.sqlite3")
  Write-Host "Applied packaged users.sqlite3."
}

$packagedManufacturerAliasesDb = Join-Path $extractDir "web_app2\manufacturer_aliases\aliases.sqlite3"
if (Test-Path $packagedManufacturerAliasesDb -PathType Leaf) {
  Copy-SqliteDatabase -Source $packagedManufacturerAliasesDb -Destination (Join-Path $InstallDir "web_app2\manufacturer_aliases\aliases.sqlite3")
  Write-Host "Applied packaged manufacturer aliases database."
}
...
if ($UsersDbPath) {
  ...
  Copy-SqliteDatabase -Source $UsersDbPath -Destination (Join-Path $InstallDir "web_app2\auth_data\users.sqlite3")
}
