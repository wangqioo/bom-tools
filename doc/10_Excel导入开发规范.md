...
本规范适用于 `web_app2` 中所有需要读取本地 Excel 文件的新接口、页面预览、批量处理和导入功能。目标是让标准 `.xls` 与 `.xlsx` 在所有 Web 功能中走同一套处理流程。
...
本规范的公共上传入口针对 Flask Web 应用。新建独立桌面或命令行脚本时，不能因为文件选择框允许 `.xls` 就直接使用 `openpyxl`；应复用同等的 `.xls` 转换逻辑，或明确只接受 `.xlsx` 并在文件选择框和错误提示中限制。现有独立脚本的兼容改造应单独纳入功能需求与测试。
